...
bot.command('setup', async ctx => {
    ...
    ctx.reply(`On a computer go to ${process.env.URL}/setup and use code <b>${code}</b>`, {
        parse_mode: 'html',
    });
});
...
export const expressMiddleware = bot.webhookCallback(process.env.TELEGRAM_ENDPOINT);